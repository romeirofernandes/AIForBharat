...
const colorMap = {
    default: 'from-foreground/85 to-foreground',
    primary: 'from-primary/85 to-primary',
...
const textShadowMap = {
    default: '[text-shadow:0_1px_0_rgba(0,0,0,0.35)]',
    primary: '[text-shadow:0_1px_0_color-mix(in_srgb,var(--primary),black_50%)]',
...
const RichButton = React.forwardRef(
    ({ children, color = 'default', size = 'default', className, asChild = false, ...props }, ref) => {
        const Comp = asChild ? Slot : 'button';
        const colorClasses = colorMap[color];
        const textShadowClasses = textShadowMap[color];
        const sizeClasses = sizeMap[size];
        const textColor = color === 'default' ? 'text-background' : color === 'primary' ? 'text-primary-foreground' : 'text-white';
        ...
        return (
            ...
            >
                {asChild
                    ...
                    : <div className={cn('relative flex items-center gap-2', textShadowClasses)}>{children}</div>
                }
            </Comp>
        );
    }
